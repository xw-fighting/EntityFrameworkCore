...
ORDER BY [c3].[CustomerID]",
                //
                @"SELECT [c5].[CustomerID], [c5].[Address], [c5].[City], [c5].[CompanyName], [c5].[ContactName], [c5].[ContactTitle], [c5].[Country], [c5].[Fax], [c5].[Phone], [c5].[PostalCode], [c5].[Region]
...
WHERE [o].[OrderID] < 10300");
        ...
        public override void Select_DTO_constructor_distinct_with_navigation_translated_to_server()
        {
            ...
            AssertSql(
                @"SELECT DISTINCT [o.Customer].[City]
FROM [Orders] AS [o]
...
FROM [Orders] AS [o]");
        ...
        public override async Task Select_nested_collection_in_anonymous_type(bool isAsync)
        {
            await base.Select_nested_collection_in_anonymous_type(isAsync);
...
WHERE [e].[Title] = N'Sales Representative'");
        ...
        public override async Task Select_Property_when_shaow_unconstrained_generic_method(bool isAsync)
        {
            await base.Select_Property_when_shaow_unconstrained_generic_method(isAsync);
...
WHERE ([o1].[CustomerID] LIKE N'A' + N'%' AND (LEFT([o1].[CustomerID], LEN(N'A')) = N'A')) AND (([ti.Customer].[CustomerID] = [ti.Customer.Customer].[CustomerID]) OR ([ti.Customer].[CustomerID] IS NULL AND [ti.Customer.Customer].[CustomerID] IS NULL))
ORDER BY [Id1], [Id2]");
        }
...
FROM [Orders] AS [o]
LEFT JOIN [Customers] AS [o.Customer] ON [o].[CustomerID] = [o.Customer].[CustomerID]
WHERE [o.Customer].[CustomerID] = @_outer_CustomerID",
                //
...
FROM [Customers] AS [c]");
        ...
        public override void Manual_expression_tree_typed_null_equality()
        {
            ...
            AssertSql(
                @"SELECT CASE
    WHEN [o.Customer].[CustomerID] IS NULL
...
WHERE [o].[OrderID] < 10300");
    ...
    SELECT COUNT(*)
    FROM [Order Details] AS [od0]
    WHERE ([o].[OrderID] = [od0].[OrderID]) AND ([od0].[Quantity] < CAST(10 AS smallint))
) AS [Count]
FROM [Orders] AS [o]
WHERE EXISTS (
    SELECT 1
    FROM [Order Details] AS [od]
    WHERE ([o].[OrderID] = [od].[OrderID]) AND ([od].[Quantity] < CAST(10 AS smallint)))");
        }
...
FROM [Customers] AS [c0]",
                //
                @"SELECT [t0].[CustomerID], [t0].[OrderDate]
FROM (
